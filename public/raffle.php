...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', sans-serif;}
        ...
        .ticket-btn--selected { transform: scale(1.15) translateY(-4px); box-shadow: 0 0 0 4px rgba(59,130,246,0.5), 0 10px 20px rgba(59,130,246,0.4); border-color: white; z-index: 10; animation: pulse 2s infinite; }
        ...
        .spinner { border: 4px solid rgba(255,255,255,0.1); border-top: 4px solid #3b82f6; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; }
        ...
        input:focus, select:focus { border-color: #3b82f6; box-shadow: 0 0 0 4px rgba(59,130,246,0.2); outline: none; }
...
<body class="bg-[#0f172a] text-slate-200">
    <header class="glass sticky top-0 z-50">
        <nav class="container mx-auto px-4 h-20 flex items-center justify-between">
            <a href="/" class="flex items-center gap-2 text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-emerald-400">
                <span class="text-2xl md:text-3xl drop-shadow-lg">&#127903;&#65039;</span>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="lg:col-span-5">
                    <div class="relative rounded-3xl overflow-hidden mb-6 border border-slate-700/50 shadow-2xl group">
                        ...
                        <div id="sold-percentage-badge" class="absolute top-4 right-4 px-4 py-2 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-full font-bold text-sm shadow-lg z-20">
                            0% vendido
                        </div>
                    </div>
                    ...
                    <div class="glass rounded-3xl p-8 mb-6">
                        ...
                        <p id="raffle-city" class="text-slate-400 mb-8 font-medium">📍 Ciudad</p>
                        ...
                        <div class="grid grid-cols-3 gap-4 mb-8 pb-8 border-b border-white/10">
                            <div>
                                ...
                                <div id="ticket-price" class="text-2xl font-black text-blue-400 font-mono">$0</div>
                            </div>
                            <div>
                        ...
                        <div class="mb-8 p-6 bg-slate-900/50 rounded-2xl border border-white/5">
                            <div class="flex justify-between mb-3 text-sm font-medium">
                                ...
                                <span id="sold-count" class="text-blue-400">0 / 0</span>
                            </div>
                            <div class="h-3 bg-slate-800 rounded-full overflow-hidden shadow-inner">
                                <div id="progress-fill" class="h-full bg-gradient-to-r from-blue-500 to-emerald-500 rounded-full transition-all duration-1000 ease-out" style="width: 0%"></div>
                            </div>
                        </div>

                        <div class="grid grid-cols-4 gap-4 mb-8">
                            <div class="countdown-box shadow-xl">
                                <div id="days" class="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-br from-blue-400 to-teal-400">0</div>
                                <div class="text-xs text-slate-400 uppercase tracking-wider mt-1">Días</div>
                            </div>
                            <div class="countdown-box shadow-xl">
                                <div id="hours" class="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-br from-blue-400 to-teal-400">0</div>
                                <div class="text-xs text-slate-400 uppercase tracking-wider mt-1">Hrs</div>
                            </div>
                            <div class="countdown-box shadow-xl">
                                <div id="minutes" class="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-br from-blue-400 to-teal-400">0</div>
                                <div class="text-xs text-slate-400 uppercase tracking-wider mt-1">Min</div>
                            </div>
                            <div class="countdown-box shadow-xl">
                                <div id="seconds" class="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-br from-blue-400 to-teal-400">0</div>
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-3">
                                <button onclick="shareRaffle('whatsapp')" class="px-4 py-2.5 bg-[#25D366]/20 text-[#25D366] border border-[#25D366]/30 rounded-xl text-sm font-bold hover:bg-[#25D366] hover:text-white transition-colors flex items-center gap-2">💬 WhatsApp</button>
                                <button onclick="shareRaffle('facebook')" class="px-4 py-2.5 bg-[#1877F2]/20 text-[#1877F2] border border-[#1877F2]/30 rounded-xl text-sm font-bold hover:bg-[#1877F2] hover:text-white transition-colors flex items-center gap-2">📘 Facebook</button>
                                <button onclick="copyLink()" class="px-4 py-2.5 bg-slate-700 text-white rounded-xl text-sm font-bold hover:bg-slate-600 transition-colors flex items-center gap-2">📋 Copiar Link</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-overlay__content border-t-4 border-t-blue-500">
            ...
            <div class="bg-slate-800/50 rounded-2xl p-6 mb-8 text-center border border-slate-700 relative overflow-hidden">
                <div class="absolute inset-0 bg-gradient-to-r from-blue-500/10 to-emerald-500/10 z-0"></div>
                <div class="relative z-10">
                    ...
                    <div class="inline-block bg-slate-900/80 px-4 py-2 rounded-lg border border-slate-700">
                        <p class="text-slate-400 text-xs">Juegas con los números:<br><span id="modal-opportunities" class="text-emerald-400 font-mono text-sm tracking-widest">--</span></p>
                    </div>
                </div>
            </div>
            ...
            <form id="purchase-form" class="space-y-5">
                ...
                <div>
                    ...
                    <small class="text-blue-400 text-xs mt-2 block flex items-center gap-1">ℹ️ Recibirás los resultados aquí</small>
                </div>
                ...
                <button type="submit" id="reserve-btn" class="w-full py-4 mt-4 bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-black rounded-xl text-lg hover:brightness-110 disabled:opacity-50 transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_25px_rgba(16,185,129,0.5)]">
                    Ir a Pagar Seguro 🔒
                </button>
            </form>
            </div>
            ...
            mobileMenuBtn.addEventListener('click', () => {
                ...
                navMenu.classList.toggle('hidden'); // Toggle 'hidden' class for initial display state
            });
            }

            // Function to share raffle link on WhatsApp (used in raffle.php)
            function shareOnWhatsApp(event) {
            event.preventDefault();
            const link = window.location.href;
            const whatsappLink = `https://wa.me/?text=¡Mira esta rifa! ${link}`;
            window.open(whatsappLink, '_blank');
            }

            // Function to copy link to clipboard (used in raffle.php)
            function copyLink() {
            const link = window.location.href;
            navigator.clipboard.writeText(link).then(() => {
                ...
                alert('Error al copiar el link. Intenta manualmente.');
            });
            }
            </script>
            </body>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="lg:col-span-7">
                        <div class="glass rounded-3xl shadow-2xl p-6 md:p-8 sticky top-24">
                            ...
                            <div id="selected-info" class="bg-indigo-900/30 border border-indigo-500/30 rounded-2xl p-6 mb-8 text-center backdrop-blur shadow-inner">
                            ...
                            <div id="multi-selection-summary" class="hidden bg-gradient-to-r from-blue-900/40 to-indigo-900/40 border border-blue-500/40 rounded-2xl p-6 mb-6 backdrop-blur shadow-xl">
                                <div class="flex justify-between items-center mb-4">
                                    ...
                                    <div class="text-right">
                                        ...
                                        <p class="text-3xl font-black text-emerald-400 font-mono" id="selected-total">$0</p>
                                    </div>
                                </div>
                                <div id="selected-numbers-display" class="flex flex-wrap gap-2 mb-4"></div>
                                <button id="pay-selected-btn" class="w-full py-4 bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-black rounded-xl text-lg hover:brightness-110 disabled:opacity-50 disabled:grayscale transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:shadow-[0_0_25px_rgba(16,185,129,0.5)]">
                                    Pagar números seleccionados →
                                </button>
                            </div>

                            <button id="continue-btn" class="w-full py-5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-black rounded-2xl text-xl hover:from-blue-500 hover:to-indigo-500 disabled:opacity-50 disabled:grayscale transition-all shadow-lg hover:shadow-blue-500/25 hidden">
                                Completar Compra →
                            </button>
                        </div>
                    </div>
    ...
    <footer class="bg-[#0b1120] text-slate-500 py-10 border-t border-slate-800 mt-20">
        <div class="container mx-auto px-4 text-center">
            <span class="text-2xl mb-4 block drop-shadow-lg opacity-50">🎟️</span>
    ...
    function renderRaffleDetails() {
        ...
        if (uniqueImages.length > 0) {
            track.innerHTML = uniqueImages.map(url => 
                `<div class="min-w-full flex-shrink-0 flex items-center justify-center bg-slate-800"><img src="${url}" alt="${r.name || ''}" class="max-w-full max-h-[500px] w-auto h-auto object-contain"></div>`
            ).join('');
...
function updateSelectionSummary() {
    ...
    numbersDisplay.innerHTML = selectedTickets.map(t =>
        `<span class="px-3 py-1 bg-blue-600/30 text-blue-300 rounded-lg font-mono text-sm border border-blue-500/30">${t.ticket_number}</span>`
    ).join('');
