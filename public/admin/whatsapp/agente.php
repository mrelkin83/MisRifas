...
<div class="grid lg:grid-cols-2 gap-4">
  <div class="neon-card p-4 space-y-3">
    ...
    <div><label class="text-xs text-[var(--text-muted)] block mb-1">Rol</label>
      <input id="rol" class="neon-input w-full" maxlength="200" placeholder="Asesor de las rifas de la plataforma"></div>
    <div><label class="text-xs text-[var(--text-muted)] block mb-1">Objetivo</label>
      <textarea id="objetivo" rows="2" class="neon-input w-full" placeholder="Resolver dudas de las rifas, ayudar a apartar números y acompañar el pago"></textarea></div>
  ...
  <div class="neon-card p-4 space-y-3">
    ...
    <textarea id="instrucciones" rows="8" class="neon-input w-full"
      placeholder="Ej: los comprobantes de pago se envían por este mismo chat. Los resultados salen la noche del sorteo con la lotería oficial. No apartamos números sin nombre y teléfono."></textarea>
    <p class="text-xs text-[var(--text-muted)]">
      Lo que escribas aquí se suma a las reglas del sistema, <b>no las reemplaza</b>. El agente
      seguirá sin poder inventar precios, apartar números vendidos, prometer premios ni dar un
      pago por confirmado, aunque se lo pidas aquí.
    </p>
  ...
  <div class="neon-card p-4 lg:col-span-2">
    ...
    <p class="text-xs text-[var(--text-muted)] mb-3">
      Estas son las acciones del agente sobre las rifas: apaga las que no quieras ofrecer.
      Las marcadas como <i>siempre</i> no se pueden apagar.
    </p>
...
async function cargar(){
  ...
  herramientas.innerHTML = TOOLS.map(t => {
    ...
    return `<label class="flex items-start gap-2 text-sm p-2 rounded border border-[var(--border-color)]">
      ...
      <span><b>${WA.esc(t.titulo || t.nombre)}</b>${t.siempre ? ' <span class="text-xs text-emerald-400">(siempre)</span>' : ''}
      <br><span class="text-xs text-[var(--text-muted)]">${WA.esc(t.descripcion)}</span></span></label>`;
  }).join('');
}

async function guardar(){
  // Siempre se manda la lista explícita: "null = todas las del motor"
  // re-abriría herramientas ajenas al dominio (cocina, garantías…).
