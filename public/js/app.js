...
const Raffles = {
    ...
    renderRaffles(raffles) {
        console.log('renderRaffles called with:', raffles);
    ...
    share(raffleId, platform) {
        const url = encodeURIComponent(`${window.location.origin}/raffle.html?id=${raffleId}`);
...
const Tickets = {
    ...
    async reserveTicket(raffleId, userData) {
        ...
        try {
            const ticket = this.selectedTickets[0]; // Por ahora solo uno
            ...
            if (response.success) {
                ...
                window.location.href = `/payment.html?ticket=${response.data.ticket.id}`;
            }
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('MisRifas initialized');
